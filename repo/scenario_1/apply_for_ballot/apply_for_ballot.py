...
def apply_for_ballot():
    ...
    #except Exception as e:
        ...
        print(f'[NETS] Payment service unavailable: {e}. Using dev fallback.')
        transaction_id = f'DEV-TXN-{applicant_id}-{int(__import__("time").time())}'
        pay_resp = type('obj', (object,), {'status_code': 200})()
        pay_data = {'data': {'transaction_id': transaction_id}}
...
def recheck_eligibility(application_id):
    ...
    try:
        ...
        elig_data = elig_resp.json()
    #except Exception as e:
        #return jsonify({'code': 503, 'message': f'Eligibility Service unavailable: {e}'}), 503 (for dev)
    except Exception as e:
        print(f'[NETS] Payment service unavailable: {e}. Using dev fallback.')
        transaction_id = f'DEV-TXN-{applicant_id}-{int(__import__("time").time())}'
        pay_resp = type('obj', (object,), {'status_code': 200})()
