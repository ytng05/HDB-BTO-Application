...
# ============================================================
# SCENARIO 2
# GET /flats/available-counts - Get available flat counts for all projects
    ...
    try:
        ...
        query = """
            SELECT
                ...
                p.project_name,
                p.town AS town_name,
                COUNT(f.flat_id) AS available_flats
            FROM bto_project p
            LEFT JOIN flat f
        ...
        query += """
            GROUP BY p.project_id, p.project_name, p.town
        ...
        counts = []
        total_available_flats = 0
        for row in rows:
            available_flats = int(row.get('available_flats') or 0)
            total_available_flats += available_flats
            counts.append({
        ...
        return jsonify({
            ...
            "data": {
                "counts": counts,
                "total_available_flats": total_available_flats
            }
        }), 200
