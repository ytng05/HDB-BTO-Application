...
def send_sms(to_mobile: str, message: str) -> None:
    ...
    print(f"[SMS OK] Sent to {formatted_mobile}")
...
def callback(ch, method, properties, body):
    try:
        ...
        message = extract_notification_payload(decoded)

        event_type = message.get("eventType")
        email = normalize_email(message.get("email"))
        mobile = normalize_mobile(message.get("mobile"))
        text = str(message.get("message", "Notification received."))
        ...
        if should_skip_external_delivery(email, mobile):
            ...
            print(f"[OK] Processed event: {event_type}")
        ...
        if email:
            try:
                ...
                delivery_errors.append(f"email delivery failed: {exc}")

        if mobile:
            try:
                send_sms(mobile, text)
            except Exception as exc:
                delivery_errors.append(f"sms delivery failed: {exc}")

        if delivery_errors:
            print("[NOTIFY] Delivery completed with errors:")
