...
def notify_apply_outcome(workflow):
    ...
    subject = "Update on Your BTO Application"
    ...
    flat_type = application.get("flat_type") if isinstance(application.get("flat_type"), str) else "selected flat type"
    ...
    for recipient in recipients:
        ...
        recipient_nric = recipient.get("nric") or main_applicant_nric
        role_label = "main applicant" if recipient.get("role") == "MAIN_APPLICANT" else "co-applicant"

        if is_eligible:
            friendly_summary = (
                f"Hi {recipient_name},\n\n"
                f"Your BTO application for {flat_type} has passed the eligibility review.\n"
                "We will keep you updated on the next steps.\n\n"
                "Regards,\nHDB"
            )
        elif formatted_ineligibility_reasons:
            friendly_summary = (
                f"Hi {recipient_name},\n\n"
                f"Your BTO application for {flat_type} did not pass the eligibility review.\n"
                "Reason(s):\n"
        ...
        if publish_event(routing_key, payload):
            sent_count += 1
        else:
            failed_count += 1
