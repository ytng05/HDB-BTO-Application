...
def initiate_select_flat():
    ...
    # Step 6: Check flat availability first
    try:
    ...
    if flat_data['data']['status'] != 'available':
        logger.warning(
            '[DEBUG] Flat %s status is not available: %s | Full flat_data: %s',
            flat_id, flat_data['data']['status'], flat_data['data']
        )
        return jsonify({
            'error': (
    ...
    try:
        ...
        logger.error('Failed to reserve flat on initiate: %s', e)
        return jsonify({'error': f'Failed to reserve flat: {e}'}), 503

    if reserve_resp.status_code != 200:
        logger.error('Flat reservation failed on initiate: %s', reserve_data.get('message'))
        return jsonify({'error': reserve_data.get('message', 'Failed to reserve flat.')}), 409

    # Initiate NETS payment
    pay = {}
    merchant_txn_ref = None
    try:
        ...
        return jsonify({'error': f'Payment Service unavailable or failed: {e}'}), 503

    # Start a background timer to unreserve after 5 minutes if payment not confirmed
    import threading, time
    def auto_unreserve_flat(merchant_txn_ref, flat_id):
        time.sleep(300)  # 5 minutes
        workflow = workflows.get(merchant_txn_ref)
        if workflow and workflow.get('stage') not in (STAGE_PAYMENT_SUCCESS, STAGE_COMPLETED, STAGE_FLAT_RESERVED):
            try:
                unreserve_resp = requests.put(
                    f'{FLAT_SERVICE_URL}/flats/{flat_id}/unreserve',
                    timeout=REQUEST_TIMEOUT,
                )
                logger.info('Auto-unreserved flat %s after timeout. Status: %s', flat_id, unreserve_resp.status_code)
            except Exception as e:
                logger.error('Failed to auto-unreserve flat %s: %s', flat_id, e)

    threading.Thread(target=auto_unreserve_flat, args=(merchant_txn_ref, flat_id), daemon=True).start()
...
def complete_select_flat(merchant_txn_ref):
    ...
    # Payment succeeded — flat is already reserved from initiate step
